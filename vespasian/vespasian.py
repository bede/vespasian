...
def infer_gene_tree(alignment_path, tree_path, output_path, separator="|"):
    ...
    stems_names = {
        r.id.partition(separator)[0]: r.id for r in records
    ...
    if len(records) == 0:
        ...
        warnings.warn(
            f"Fewer than 7 sequences found in {alignment_path}. Consider exclusion."
        )
...
def infer_gene_trees(input_path, tree_path, output_path, separator="|", progress=False):
    ...
    observed_stems = set()
    for family, path in tqdm.tqdm(
        families_paths.items(), desc="Inferring trees", disable=not progress
    ):
    ...
    if observed_stems.difference(species_tree_stems):
        raise NameError(
            f"The following taxa were not found in the species tree: \n"
...
def prune_redundant_branches(branches, tree_path, separator):
    ...
    branches_leaves_present = {
        b: l.intersection(taxa_present) for b, l in branches_leaves.items()
    }

    supertrees_subtrees = {
        i: j
        for i, j in permutations(branches_leaves, 2)
    ...
    for supertree, subtree in supertrees_subtrees.items():
        ...
        warnings.warn(
            f"Skipped labelling redundant supertree {supertree} for {tree_path}"
        )

    return {
        b: list(l) for b, l in branches_leaves.items()
    }  # values() from sets to lists
...
def label_branch(tree_path, branch_label, leaf_labels, separator="|", strict=False):
    ...
    tree = treeswift.read_tree_newick(tree_path)
    taxa_longnames = {
        n.label.partition(separator)[0]: n.label for n in tree.traverse_leaves()
    }
    taxa = set(taxa_longnames.keys())

    if leaf_labels:  # Branch is internal node
        tree_leaf_intersection = set(taxa_longnames.keys()).intersection(
            leaf_labels
        )  # Coerces to set
        intersection_size = len(tree_leaf_intersection)
        required_taxa = (
            len(leaf_labels) if strict else 2
        )  # Ignore singletons by default

        if intersection_size < required_taxa:
            warnings.warn(
                f"Insufficient taxa present to label branch {branch_label} in {tree_path}"
            )
            raise RuntimeError()

        expanded_leaf_labels = (
            taxa_longnames.get(l) for l in leaf_labels
        )  # Generate long names
        ...
        if branch_label not in taxa:
            warnings.warn(
                f"Insufficient taxa present to label leaf {branch_label} in {tree_path}"
            )
            raise RuntimeError()
...
def setup_branch_site_models(
    ...
    models_omega = {"modelA": [0, 1, 2, 10], "modelAnull": [1]}
    ...
    if branches:
        ...
        childbearing_branches_pruned = prune_redundant_branches(
            childbearing_branches, f"{family_path}/tree.nwk", separator
        )
        ...
        for branch, leaves in branches_pruned.items():
            ...
            try:
                labelled_tree = label_branch(
                    f"{family_path}/tree.nwk", branch, leaves, separator, strict
                )
...
def gather_codeml_commands(path):
    ...
    codeml_dirs_no_prefix = [
        Path(p).relative_to(path) for p in codeml_dirs
    ]  # Relative to codeml/
...
def setup_family(
    family, alignment_path, output_dir, gene_trees_dir, branches, separator, strict
):
    ...
    setup_branch_site_models(
        family, family_path, alignment_path, gene_tree_path, branches, separator, strict
    )
...
def codeml_setup(
    ...
    alignment_paths = [
        f"{families_dir}/{fn}"
        for fn in os.listdir(families_dir)
        if fn.endswith((".fa", ".fasta"))
    ]
...
def parse_result(path):
    ...
    try:
        ...
        for line in result_lines:
            if line.startswith("lnL"):
                ...
                if line.startswith("proportion"):
                    ...
                    p_labels_params = {
                        f"p{p_label}": p_param
                        for p_label, p_param in zip(p_labels, p_params)
                    }
                    ...
                    w_labels_params = {
                        f"w{w_label}": w_param
                        for w_label, w_param in zip(w_labels, w_params)
                    }
                ...
                if line.startswith("omega"):
                    ...
                    p_params = {
                        f"p{p_label}": p_param
                        for p_label, p_param in zip(
                            p_labels, tuple(map(float, p_record))
                        )
                    }
                    ...
                    w_params = {
                        f"w{w_label}": w_param
                        for w_label, w_param in zip(
                            w_labels, tuple(map(float, w_record))
                        )
                    }
        ...
        if model in ("m0", "m2Selection", "m3Discrtk2", "m3Discrtk3", "m8"):
            neb_pos_site_lines = re.findall(
                r"Naive Empirical Bayes \(NEB\).*?mean \+\- SE for w(.*?)\n\n\n",
                result_contents,
                re.DOTALL,
            )
            beb_pos_site_lines = re.findall(
                r"Bayes Empirical Bayes \(BEB\).*?mean \+\- SE for w(.*?)\n\n\n",
                result_contents,
                re.DOTALL,
            )
        ...
        if model == "modelA":
            ...
            neb_pos_site_lines = re.findall(
                r"Naive Empirical Bayes \(NEB\).*?Prob\(w\>1\)\:\n(.*?)\n\n",
                result_contents,
                re.DOTALL,
            )
            beb_pos_site_lines = re.findall(
                r"Bayes Empirical Bayes \(BEB\).*?Prob\(w\>1\)\:\n(.*?)\n\n",
                result_contents,
                re.DOTALL,
            )
            if neb_pos_site_lines:
                neb_lines = neb_pos_site_lines[0].strip().split("\n")  # Item per line
                neb_lines = [
                    l.strip("* ") for l in filter(None, neb_lines)
                ]  # Filter empty str and strip
                # print(path, 'branch_site_neb', neb_lines)
                neb_records = [
                    {"position": int(r[0]), "residue": r[1], "p": float(r[2])}
            ...
            if beb_pos_site_lines:
                beb_lines = beb_pos_site_lines[0].strip().split("\n")  # Item per line
                beb_lines = [
                    l.strip("* ") for l in filter(None, beb_lines)
                ]  # Filter empty str and strip
...
def parse_results(input_dir, progress=False):
    ...
    results = [
        parse_result(path) for path in tqdm.tqdm(output_paths, disable=not progress)
    ]
    ...
    for family, tree in families_branches:
        if family == tree:  # Site models
            for (null, alt), meta in site_lrts.items():
                ...
                critical_value = (
                    chi2.ppf(1 - 5e-2, df=meta["ddof"]) if meta["ddof"] else 0
                )  # 3.84, 5.99 etc.
